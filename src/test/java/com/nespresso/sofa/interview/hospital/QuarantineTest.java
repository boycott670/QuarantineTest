...
import com.nespresso.quarantine.Quarantine;
...
public class QuarantineTest
{
   ...
   * group of patients. It is initialized with a list of patients' health status,
   * separated by a comma. Each health status is described by one or more
   * characters (in the test below, we will always have only one disease /
   * patient)
   *
   * The characters mean: H : Healthy F : Fever D : Diabetes T : Tuberculosis
   *
   * Quarantine provides medicines to the patients, but can not target a specific
   * group of patient. The same medicines are always given to all the patients.
   *
   * Then Quarantine can provide a report with this format: "F:1 H:2 D:0 T:1 X:3"
   *
   * Report give the number of patients that have the given disease. X means Dead
   *
   */
